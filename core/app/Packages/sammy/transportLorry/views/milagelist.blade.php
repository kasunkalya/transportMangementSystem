@extends('layouts.sammy_new.master') @section('title','Milage List')
@section('css')
<style type="text/css">
	.switch.switch-sm{
		width: 30px;
	...
	<div class="col-xs-12">
		<div class="panel panel-bordered">
      		<div class="panel-heading border">
				<div class="row">
					<div class="col-xs-6"><strong>Milage List</strong></div>
					<div class="col-xs-6 text-right"><a href="{{{url('transportLorry/addMilage')}}}"><button class="btn btn-success"><i class="fa fa-plus" style="width: 28px;"></i>Milage Add</button></a></div>
				</div>
      		</div>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		table = generateTable('.datatable', '{{url('transportLorry/json/milagelist')}}',[],[]);
		...
		table.on('draw.dt',function(){
			...
			$('.type-delete').click(function(e){
				e.preventDefault();
				id = $(this).data('id');
				sweetAlertConfirm('Delete Milage Record', 'Are you sure?',2, deleteFunc);
			});
		});
	});
	 ...
	 */
	function deleteFunc(){
		ajaxRequest( '{{url('transportLorry/deletemilage')}}' , { 'id' : id  }, 'post', handleData);
	}
