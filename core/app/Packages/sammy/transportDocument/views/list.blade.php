@extends('layouts.sammy_new.master') @section('title','Transport Document Layout List')
@section('css')
<style type="text/css">
	.switch.switch-sm{
		width: 30px;
	...
	<div class="col-xs-12">
		<div class="panel panel-bordered">
      		<div class="panel-heading border">
				<div class="row">
					<div class="col-xs-6"><strong>Document Layout</strong></div>
					<div class="col-xs-6 text-right"><a href="{{{url('transportDocument/add')}}}"><button class="btn btn-success"><i class="fa fa-plus" style="width: 28px;"></i>Document Layout Add</button></a></div>
				</div>
      		</div>
          		...
          		<table class="table table-bordered bordered table-striped table-condensed datatable">
	              	<thead>
		                <tr>
									...
									<th rowspan="2" class="text-center" style="font-weight:normal;">Document Name</th>  
                                    <								  
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		table = generateTable('.datatable', '{{url('transportDocument/json/list')}}',[],[]);
		...
		table.on('draw.dt',function(){
			...
			$('.type-delete').click(function(e){
				e.preventDefault();
				id = $(this).data('id');
				sweetAlertConfirm('Delete Document Layout', 'Are you sure?',2, deleteFunc);
			});
		});
	});
	 ...
	 */
	function deleteFunc(){
		ajaxRequest( '{{url('transportDocument/delete')}}' , { 'id' : id  }, 'post', handleData);
	}
