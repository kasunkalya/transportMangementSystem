@extends('layouts.sammy_new.master') @section('title','Transport Add Invoice')
@section('css')
<style type="text/css">
	.switch.switch-sm{
		width: 30px;
...
<ol class="breadcrumb">
	<li>
  	...
  	<li class="active">
		<a href="{{{url('transportManagement/addInvoice')}}}">Add New Invoice</a>
  	</li>
				...
				<div class="panel-body" id="content">
					...
					<table class="table table-bordered bordered table-striped table-condensed datatable">
						<thead>
							<tr>
								...
								<th rowspan="2" class="text-center" style="font-weight:normal;">Lorry</th>
                                                                <th rowspan="2" class="text-center" style="font-weight:normal;">Gate Pass No</th>
                                                                <th rowspan="2" class="text-center" style="font-weight:normal;">Total</th>
								...
								<th style="display: none;" width="2%"></th>								
                                        ...
                                        <div class="panel-body">
                                            ...
                                            <div class="form-group">
                                                    <label class="col-sm-2 control-label required">Customer</label>
                                                    <div class="col-sm-10">
                                                            @if($errors->has('invoiceCustomer'))
                                                                    {!! Form::select('invoiceCustomer',$customer, Input::old('invoiceCustomer'),['placeholder' => 'Select customer','class'=>'chosen error','style'=>'width:100%;','required','data-placeholder'=>'Set After','id'=>'invoiceCustomer']) !!}
                                                                    <label id="supervisor-error" class="error" for="route">{{$errors->first('route')}}</label>
                                                            @else
                                                                    {!! Form::select('invoiceCustomer',$customer, Input::old('invoiceCustomer'),['placeholder' => 'Select customer','class'=>'chosen','style'=>'width:100%;','required','data-placeholder'=>'Set After','id'=>'invoiceCustomer']) !!}
                                                            @endif
						...
						<div class="col-sm-10">						
                                            ...
                                            <div class="form-group">
                                                    <label class="col-sm-2 control-label required">VAT</label>
                                                    <div class="col-sm-10">
                                                        <input type="text"  id="vat" class="form-control @if($errors->has('vat')) error @endif" name="vat" placeholder="VAT %" required value="{{Input::old('vat')}}" >
                                                            @if($errors->has('vat'))
            ...
            for (var i=0; i<len; i++) {
               if(cboxes[i].checked){
                    ...
                    $.ajax({
                        ...
                        url: 'createInvoice/'+result+'/'+vat+'/'+customerValue+'/'+customerValue+'/'+routeValue+'/'+companyValue,
                        type: 'GET',                  
                        dataType: 'JSON',
                        ...
                        success: function (data) { 
								...
								//pdf-create/'. $value->id.'/'.$customer['invoiceLayoutId'].
                               window.open('{{url('pdf-create/')}}/'+data['invoiceid']+'/'+data['layoutid']);
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		table = generateTable('.datatable', '{{url('transportManagement/json/tripsearchlist')}}/'+0+'/'+0+'/'+0+'/'+0+'/'+0+'/'+0+'/'+0,[],[]);
...
<script type="text/javascript">
        ...
        function loadTable(){
                  ...
                  if(optradio==4){
                    ...
                    table.destroy();
                    table = generateTable('.datatable', '{{url('transportManagement/json/tripsearchlist')}}/'+optradio+'/'+invoicedatefrom+'/'+invoicedateto+'/'+lorryid+'/'+invoice+'/'+customerid+'/'+route,[],[]);
                    $("#content").show();
