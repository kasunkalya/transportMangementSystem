@extends('layouts.sammy_new.master') @section('title','Fuel Record List')
@section('css')
<style type="text/css">
	.switch.switch-sm{
		width: 30px;
...
<ol class="breadcrumb">
	<li>
  	...
  	<li>
    	<a href="{{{url('transportLorryFule/list')}}}">Fuel Record List</a>
  	</li>
</ol>
<div class="row">
	<div class="col-xs-12">
		<div class="panel panel-bordered">
      		<div class="panel-heading border">
				<div class="row">
					<div class="col-xs-6"><strong>Fuel Record List</strong></div>
					<div class="col-xs-6 text-right"><a href="{{{url('transportLorryFule/add')}}}"><button class="btn btn-success"><i class="fa fa-plus" style="width: 28px;"></i>Fuel Record Add</button></a></div>
				</div>
      		</div>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		table = generateTable('.datatable', '{{url('transportLorryFule/json/list')}}',[],[]);
		...
		table.on('draw.dt',function(){
			...
			$('.type-delete').click(function(e){
				e.preventDefault();
				id = $(this).data('id');
				sweetAlertConfirm('Delete Fuel Record', 'Are you sure?',2, deleteFunc);
			});
		});
	});
	 ...
	 */
	function deleteFunc(){
		ajaxRequest( '{{url('transportLorryFule/delete')}}' , { 'id' : id  }, 'post', handleData);
	}
