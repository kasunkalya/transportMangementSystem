@extends('layouts.sammy_new.master') @section('title','Repair Record List')
@section('css')
<style type="text/css">
	.switch.switch-sm{
		width: 30px;
...
<ol class="breadcrumb">
	<li>
  	...
  	<li>
    	<a href="{{{url('transportLorryRepair/list')}}}">Repair Record List</a>
  	</li>
</ol>
<div class="row">
	<div class="col-xs-12">
		<div class="panel panel-bordered">
      		<div class="panel-heading border">
				<div class="row">
					<div class="col-xs-6"><strong>Repair Record List</strong></div>
					<div class="col-xs-6 text-right"><a href="{{{url('transportLorryRepair/add')}}}"><button class="btn btn-success"><i class="fa fa-plus" style="width: 28px;"></i>Repair Record Add</button></a></div>
				</div>
      		</div>
          		...
          		<table class="table table-bordered bordered table-striped table-condensed datatable">
	              	<thead>
		                <tr>
		                  	...
		                  	<th rowspan="2" class="text-center" style="font-weight:normal;">Repair Date</th>
		                  	<th rowspan="2" class="text-center" style="font-weight:normal;">Grage</th>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		table = generateTable('.datatable', '{{url('transportLorryRepair/json/list')}}',[],[]);
		...
		table.on('draw.dt',function(){
			...
			$('.type-delete').click(function(e){
				e.preventDefault();
				id = $(this).data('id');
				sweetAlertConfirm('Delete Lorry Repair Record', 'Are you sure?',2, deleteFunc);
			});
		});
	});
	 ...
	 */
	function deleteFunc(){
		ajaxRequest( '{{url('transportLorryRepair/delete')}}' , { 'id' : id  }, 'post', handleData);
	}
