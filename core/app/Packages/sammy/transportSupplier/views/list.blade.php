@extends('layouts.sammy_new.master') @section('title','Transport Supplier List')
@section('css')
<style type="text/css">
	.switch.switch-sm{
		width: 30px;
...
<ol class="breadcrumb">
	<li>
  	...
  	<li class="active">
		<a href="{{{url('transportSupplier/list')}}}">Supplier List</a>
  	</li>
	...
	<div class="col-xs-12">
		<div class="panel panel-bordered">
      		<div class="panel-heading border">
				<div class="row">
					<div class="col-xs-6"><strong>Supplier List</strong></div>
					<div class="col-xs-6 text-right"><a href="{{{url('transportSupplier/add')}}}"><button class="btn btn-success"><i class="fa fa-plus" style="width: 28px;"></i>Supplier Add</button></a></div>
				</div>
      		</div>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		table = generateTable('.datatable', '{{url('transportSupplier/json/list')}}',[],[]);
		...
		table.on('draw.dt',function(){
			...
			$('.type-delete').click(function(e){
				e.preventDefault();
				id = $(this).data('id');
				sweetAlertConfirm('Delete Supplier', 'Are you sure?',2, deleteFunc);
			});
		});
	});
	 ...
	 */
	function deleteFunc(){
		ajaxRequest( '{{url('transportSupplier/delete')}}' , { 'id' : id  }, 'post', handleData);
	}
